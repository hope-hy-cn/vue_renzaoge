...
export default {
  ...
  methods: {
    getData (categorylistID) {
      const categorylist = JSON.parse(window.sessionStorage.getItem('rzg-categorylist')) || {}
      const categorylistname = JSON.parse(window.sessionStorage.getItem('rzg-categorylist-name')) || {}
      if (Object.keys(categorylist).includes(categorylistID.toString())) {
        ...
        this.$http.getCategoryList(categorylistID)
          .then(resp => {
            ...
            window.sessionStorage.setItem('rzg-categorylist', JSON.stringify(categorylist))
            window.sessionStorage.setItem('rzg-categorylist-name', JSON.stringify(categorylistname))
          })
      }
    }
  },
